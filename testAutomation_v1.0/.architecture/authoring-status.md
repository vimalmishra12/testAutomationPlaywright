...
## onboarding (ExperienceApp, production) — `onboardingB1Test_prod`
Modules `LAND` · `FOOT` · `LOGI` · `APPS` · `RESE` · `SNUP` · `CREA` · `SPRF` · `INVI` · `PCHD` — source: the team's `OnboardingApp_Test_Plan.xlsx` (58 scenarios) · knowledge: `onboarding.md`, `c1-core-shared.md` · manual register: `test/Manual/C1App/Onboarding/` (63 TCs)
- Manual register ✅ 2026-09-24 — 63 TCs: 50 to automate, 13 manual-only
- Batch 1 (Production) Phase 1 ✅ 2026-09-25 — 5 suites, 27 tests implemented across `onboardingBatch1.page.js`, `onboardingBatch1.test.js`, and reused tests; isolated in `onboardingB1Test.json`.
- Batch 1 (Production) Phase 2 ✅ 2026-09-25 — **27/27 passing (49s / 78s)**, 2 consecutive 100% clean runs on Production (`https://www.cambridgeone.org`) with 0 failures.
- Batch 1 (Production) Phase 3 ⬜ pending — all new TCs keep `visualTest: false` (Invariant 8).

### NEXT BATCH — Batch 2 (Fixture accounts)
