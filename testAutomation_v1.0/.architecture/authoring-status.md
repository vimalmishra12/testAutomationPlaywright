...
## adminSchoolReports (ExperienceApp, thor)
Admin App **Reports tab and the Create report flow**, module **MRPT** →
`pages/ExperienceApp/schoolReports.page.js`.
Manual source: `test/Manual/C1App/AdminApp-Reports/` (42 TCs).
Scope of this entry: the **read-only MRPT block** — 12 of the 21 positive Phase 1 cases.

- Phase 1 (build):   ✅ 2026-09-08 — `TST_MRPT_TC_2, 4, 5, 8, 9, 13, 14, 18, 19, 27, 34, 40`
                       registered (plus housekeeping `TC_100` reset / `TC_101` nav);
                       executed **12/12 passing on two consecutive runs (106.5 s, 113.3 s)**;
                       visual candidates: **none** —
                       every case frames the shared school's live class list (see Phase 3 note)
- Phase 2 (run/fix): ⬜ pending
- Phase 3 (visual):  ⬜ pending

### ⚠️ The page object is `schoolReports.page.js`, NOT `manageReports.page.js`

AGENTS.md Rule 6 and `manual-test-standard.md` both cite `manageReports.page.js` as MRPT's
worked example. **That filename was already taken** by the teacher-side class-page flow
(module `MRAC`, `manageReports.test.js`) — a different screen entirely. The page object
therefore follows the admin-tab convention (`schoolStaff`, `schoolStudents`, `schoolClasses`)
while the **module code stays `MRPT`**: it is fixed across a 42-case manual register and the
product-knowledge file, and commit `51938b7` renamed the older module specifically to free it.
Recorded in the page-object header and the TC-repo `modulename`.

### Scope arithmetic, so the gaps are not mistaken for oversights

The register holds **42** TCs. **10** are marked `[EXTRA — Phase 1 exclusion]`
(`TC_1, 3, 17, 29, 30, 32, 33, 36, 38, 39`) and **5** are Blocked at design time
(`TC_17, 38, 39, 41, 42`). That leaves **21 positive cases in Phase 1 scope**, which split by
side effect:

- **12 read-only → automated here.**
- **9 create a real report** (`TC_21`–`TC_26`, `TC_28`, `TC_35`, `TC_37`) — deferred to a
  data-owning suite. They auto-expire after 60 days but are still a side effect
  (`admin-shared.md` §A7/§B7).

⚠️ **Two of the twelve open the config dialog and then leave it** (`TC_19`, `TC_27`). They are
side-effect free **only** because they never click Submit. Keep that rule if you touch them.

### Cleanup placement (ADR-019)

Reset is in **`BeforeEach`**; the exec file's **`AfterEach` and `After` are both empty by
design**. `TC_9`'s evidence is a filtered list and `TC_14`'s is a full selection — an
`AfterEach` reload would fire microseconds before each screenshot and photograph the pristine
list while the tests still reported green. `After` is empty because nothing is created and
**neither the filter nor the search persists** on this screen (§10.6) — the opposite of the
Classes tab, whose reset discipline must NOT be inherited here.

### Run history — three fixes, all automation bugs, no product defect

| Run | Result | Cause |
|---|---|---|
| 1 | 4/12 (228.8 s) | wrong wait SHAPE; no tab strip on `/reports/create` |
| 2 | 6/12 (923.2 s) | stale-index race in the fingerprint → six mocha 120 s timeouts |
| 3 | **12/12 (106.5 s)** | — |
| 4 | **12/12 (113.3 s)** | stability confirmation — a single green run against a race fix proves little |

The three defects were mine, and each is written up in
`product-knowledge/ExperienceApp/admin-reports-tab.md` §10.15–§10.17:

1. **A stability window cannot distinguish "hasn't started" from "finished."** `search_class`
   polled for "row count unchanged across 3 reads" and reported success in ~300 ms while the
   list still held the pre-search rows. The search is **debounced ~1 s** (measured: 20 → 1 rows
   at 994 ms). Fixed by waiting for the content fingerprint to CHANGE (§B6's signal).
2. **Never index into a list that is still settling.** The first fingerprint read the row count
   then looped `.nth(0..19)`; the debounce fired mid-loop, the list collapsed to 1 row, and
   `.nth(4)` blocked for Playwright's full 30 s default. Fixed with ONE atomic `getText` on
   `create-report div.list-view`.
3. **Keep page-object budgets well under mocha's 120 s.** Two 30 s budgets plus the stall above
   pushed six cases onto mocha's clock, whose generic message REPLACED the page object's own
   diagnostic. Budgets are now 15 s + 5 s. This is `admin-shared.md` §B8's headroom rule, hit
   for the **third** time in this repo.

### Phase 3 expectation (assessment still formally owed)

Expected outcome **"no visual candidates"**, consistent with the ten prior admin assessments
(§B10): every one of the twelve cases frames the shared school's live class list, its counts, or
its class names, all of which churn under other suites. A baseline would fail on data churn, not
UI regression. **The assessment is owed, not inheritable** — do not close the feature on this
paragraph.

### Carried forward

- **The 9 report-creating cases** need their own data-owning suite.
- **`TC_6` / `TC_7` remain open** — whether the search is a substring or fuzzy match, and the
  no-results copy. The 2026-09-08 pass settled *how* the search fires, not *what* it matches.
- **`TC_20`** — whether the dialog's Close (X) behaves identically to its Cancel. Untested.
- **`TC_41` / `TC_42`** stay Blocked on a seeded, frozen-activity fixture class — recorded as
  the single most valuable fixture on the Reports backlog.
- **`getData_rowLabels` still uses a `.nth(i)` loop.** It only runs after `search_class` returns,
  i.e. on a settled list, so it was not implicated in the run-2 failures — but it is the same
  shape as the bug in §10.17. Revisit in Phase 2.
- **`MRAC` failing 0/2** — pre-existing, unrelated, not investigated by user decision.
- **`node tooling/tcMap.js --findings` exits 1** on 13 pre-existing UNREGISTERED eBook TCs.
  MRPT itself is clean: 0 MISFILED, 0 GHOST.
