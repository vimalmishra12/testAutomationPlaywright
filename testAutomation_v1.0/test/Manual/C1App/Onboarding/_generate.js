...
**Source:** \`OnboardingApp_Test_Plan.xlsx\` (supplied by the user ${DATE}) — sheet "Test Cases", 58 scenarios in 11 sub-modules (\`TC_HOME_*\`, \`TC_LOGIN_*\`, \`TC_FPWD_*\`, \`TC_RESETPW_*\`, \`TC_ROLE_*\`, \`TC_LRN_*\`, \`TC_TCH_*\`, \`TC_PAR_*\`, \`TC_CHLD_*\`, \`TC_INVITE_*\`, \`TC_XCUT_*\`) + sheet "Legend"
...
**Generated:** ${DATE} | **Total TCs:** ${rows.length} (${counts.Positive || 0} Positive · ${counts.Edge || 0} Edge · ${counts.Negative || 0} Negative) — **57 of the source's 58 scenarios have a case**; TC_XCUT_009 (UI/colours) is a visual-layer check, not a manual case (see map)
**Execution status (${DATE}):** designed, not executed — ${Object.entries(byStatus).map(([k, v]) => "**" + v + " " + k + "**").join(" · ")}. ${existing.length} rows reuse an EXISTING automated TC (${ids(existing)}) whose assertion Phase 1 must confirm; none is counted as passing here.
...
> ${DATE}: LAND 6+, FOOT 12+, LOGI 7+, RESE 6+, SNUP 65+, CREA 31+, SPRF 24+, INVI 14+). Where an
...
async function main() {
  ...
  rows.forEach((r) => {
    const row = ws.addRow([r.sno, r.id, r.title, r.req, r.type, r.priority, r.pre, r.steps,
      r.data, r.expected, r.remarks, "", r.status, r.comments, SCOPE_TEXT[r.scope]]);
