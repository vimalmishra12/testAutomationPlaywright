...
> **[2026-09-21] Extra, non-automated cases REMOVED.** 5 cases that were "Extra in Ours" in `Admin_Gap_Analysis.xlsx` **and** not automated (Excluded – Phase 1 or Blocked) were removed from this register (.md and .xlsx): `TST_SPRF_TC_13`, `TST_SPRF_TC_20`, `TST_SBLK_TC_9`, `TST_SBLK_TC_10`, `TST_SBLK_TC_12`. Their ids are retired — never reuse them. Other mentions of these ids further down (coverage map, Remarks, Open items) are historical; the full pre-removal copies are in `archive/AdminApp_Students_tab_test_cases_before-extra-removal_2026-09-21.*`.
...
| **Automation Evidence** | Handler exists in studentProfile.test.js but no execution file runs it. |
...
| **Status** | On Hold |
| **Automation Status** | Written - not wired |
| **Automation Evidence** | Handler exists in adminStudentsTab.test.js but no execution file runs it. |
...
| **Status** | On Hold |
| **Automation Status** | Written - not wired |
| **Automation Evidence** | Handler exists in bulkStudents.test.js but no execution file runs it. |
