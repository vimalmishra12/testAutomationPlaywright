...
module.exports = {
  ...
  // ---------------------------------------------------------------------------------------------
  // LP batch 2 B/D/E (2026-09-23, prod). Scorable single-learner facts (probe on fresh learner _5an7):
  // with NO option chosen the outer Check (a.green-btn) is NOT RENDERED — it appears on the first
  // selection; a checked frame keeps its answer (`.checked-correct`) when the activity is left and
  // reopened, and the attempt continues to 4/4. So LP-013 and LP-025 run inside Suite 7's one attempt.
  // ---------------------------------------------------------------------------------------------

  /** LP-013: frame `frameIndex` (0-based) before anything is chosen — is Check offered at all? */
  getData_frameCheckState: async function (frameIndex) {
    await logger.logInto(await stackTrace.get(), "frame:" + frameIndex);
    var pe = selectorFile.css.ComproC1.practiceExtra;
    var n = String(frameIndex);
    await action.switchToFrame(pe.playerIframe);
    var frameShown = true == (await action.waitForDisplayed(pe.frameDropdown.replace(/\{N\}/g, n), 30000));
    var filled = frameShown ? await action.getElementCount(pe.frameDropdownFilled.replace(/\{N\}/g, n)) : -1;
    await action.switchToParentFrame();
    // The player chrome is ready before the frame; Check would render with it, so no extra wait is owed.
    return { frameShown: frameShown, filledCount: filled, checkShown: true == (await action.isDisplayed(pe.checkBtn)) };
  },

  /** TOC status label of one activity (its icon's aria-label / alt), e.g. "Activity status: in progress". */
  getData_activityStatus: async function (name) {
    await logger.logInto(await stackTrace.get(), "activity:" + name);
    var sel = selectorFile.css.ComproC1.practiceExtra.tocActivityStatus.replace(/\{NAME\}/g, name);
    if ((await action.getElementCount(sel)) < 1) return { status: null };
    var label = await action.getAttribute(sel, "aria-label");
    if (!label) label = await action.getAttribute(sel, "alt");
    return { status: label };
  },

  /**
   * LP-025: leave a half-done scorable for `awayName` via the TOC, then reopen it. Reports the scorable's
   * TOC status while away, where it reopens, and whether frame `frameIndex`'s checked answers survived.
   * Ends with the TOC closed and the scorable on screen, so the Suite 7 frames continue from here.
   */
  leave_and_relaunch: async function (activityName, awayName, frameIndex, unitName) {
    await logger.logInto(await stackTrace.get(), "activity:" + activityName + " away:" + awayName);
    var pe = selectorFile.css.ComproC1.practiceExtra;
    var n = String(frameIndex);
    var out = { awayTitle: null, statusWhileAway: null, backTitle: null, frameShown: false, filledCount: -1, correctCount: -1, nextShown: false };
    // [2026-09-23, full run] On a learner's FIRST entry the TOC reopens on the UNIT view, which lists no
    // activity rows — so bring it to the lesson view of the unit before each activity click.
    await this.ensure_tocLessonView(unitName);
    out.awayTitle = (await this.open_tocActivity(awayName)).title;
    out.statusWhileAway = (await this.getData_activityStatus(activityName)).status;
    await this.ensure_tocLessonView(unitName);
    out.backTitle = (await this.open_tocActivity(activityName)).title;
    await this.ensure_tocClosed();
    await action.switchToFrame(pe.playerIframe);
    out.frameShown = true == (await action.waitForDisplayed(pe.frameWrap.replace(/\{N\}/g, n), 30000));
    if (out.frameShown) {
      out.filledCount = await action.getElementCount(pe.frameDropdownFilled.replace(/\{N\}/g, n));
      out.correctCount = await action.getElementCount(pe.frameCheckedCorrect.replace(/\{N\}/g, n));
    }
    await action.switchToParentFrame();
    // A checked frame offers Next (not Check) — the attempt can carry on from where it was left.
    out.nextShown = true == (await action.waitForDisplayed(pe.nextBtn, 10000));
    return out;
  },

  /** The unit row's completion counter, e.g. "3/4 Completed" → { done: 3, total: 4 }. TOC must be at unit view. */
  getData_unitProgress: async function () {
    await logger.logInto(await stackTrace.get());
    var pe = selectorFile.css.ComproC1.practiceExtra;
    if (true != (await action.waitForDisplayed(pe.tocUnitProgress, 10000))) return { text: null, done: -1, total: -1 };
    var text = (await action.getText(pe.tocUnitProgress)).trim();
    var m = text.match(/(\d+)\s*\/\s*(\d+)/);
    return { text: text, done: m ? Number(m[1]) : -1, total: m ? Number(m[2]) : -1 };
  },

  /**
   * LP-021: open a non-scorable activity from the (open, lesson-view) TOC and wait for its TOC status to
   * reach `status` ("Activity status: viewed"). [2026-09-23, prod] The HTML activity reports "viewed" ~1.5 s
   * after it opens (20 s bound) — no submit — and "viewed" counts toward the unit's "N/4 Completed".
   */
  open_activityAndWaitStatus: async function (name, status) {
    await logger.logInto(await stackTrace.get(), "activity:" + name + " status:" + status);
    var pe = selectorFile.css.ComproC1.practiceExtra;
    var before = (await this.getData_activityStatus(name)).status;
    var opened = await this.open_tocActivity(name);
    var iframeShown = true == (await action.waitForDisplayed(pe.playerIframe, 30000));
    var now = null;
    var deadline = Date.now() + 20000;
    while (Date.now() < deadline) {
      now = (await this.getData_activityStatus(name)).status;
      if (now === status) break;
      await browser.pause(500);
    }
    return { title: opened.title, iframeShown: iframeShown, statusBefore: before, statusAfter: now };
  },

  /**
   * LP-022: from the current non-scorable activity (TOC open on its lesson view), NEXT ACTIVITY lands on a
   * downloadable activity. [2026-09-23, prod; user-confirmed behaviour] The player shows a DOWNLOAD PAGE
   * (instructions, file name, Download link) — not the PDF — and landing on it marks the activity
   * "viewed". Download is NEVER clicked. Reads the TOC status before (lesson view must be open) and after.
   */
  open_nextActivityDownload: async function (name, status) {
    await logger.logInto(await stackTrace.get(), "activity:" + name);
    var pe = selectorFile.css.ComproC1.practiceExtra;
    var out = { statusBefore: null, title: null, pageShown: false, instructions: null, fileName: null, downloadShown: false, statusAfter: null };
    out.statusBefore = (await this.getData_activityStatus(name)).status;
    await this.ensure_tocClosed();
    if (true != (await action.click(pe.nextActivityBtn))) return out;
    var deadline = Date.now() + 30000;
    while (Date.now() < deadline) {
      out.title = await action.getAttribute(pe.activityTitleBtn, "title");
      if (out.title === name) break;
      await browser.pause(250);
    }
    out.pageShown = true == (await action.waitForDisplayed(pe.downloadPage, 30000));
    if (out.pageShown) {
      out.instructions = (await action.getText(pe.downloadInstructions)).trim();
      out.fileName = (await action.getText(pe.downloadFileName)).trim();
      out.downloadShown = true == (await action.isDisplayed(pe.downloadLink));
    }
    // The TOC re-opens on the lesson view of the current activity — its row carries the status.
    await this.click_openToc();
    deadline = Date.now() + 15000;
    while (Date.now() < deadline) {
      out.statusAfter = (await this.getData_activityStatus(name)).status;
      if (out.statusAfter === status) break;
      await browser.pause(500);
    }
    return out;
  },
  ...
  /**
   * HOUSEKEEPING (TC_102): TOC open at the LESSON view of `unitName`. The open control reopens whichever
   * view was last shown (after TC_20 that is the unit view), so it is not enough to just open it.
   */
  ensure_tocLessonView: async function (unitName) {
    await logger.logInto(await stackTrace.get(), "unit:" + unitName);
    var pe = selectorFile.css.ComproC1.practiceExtra;
    if (true != (await action.isDisplayed(pe.tocSidebar))) {
      await action.click(pe.tocOpenBtn);
    ...
    if (true == (await action.isDisplayed(pe.tocUnitViewCloseBtn))) {
      await action.click(action.getFilteredLocator(pe.tocUnitItem, unitName));
    }
    return { lessonView: true == (await action.waitForDisplayed(pe.tocLessonViewCloseBtn, 10000)) };
  },

  /**
   * Teacher / admin preview of a Learning Path component (LP-028/032/033): the teacher player route
   * `/learning-path/teacher/…` with an activity loaded and a TOC that renders its content.
   * [2026-09-23, prod] The TOC opens by itself only on a user's FIRST entry (the admin's second launch
   * came up with it closed), so when it is closed it is opened with the player's open control — then it
   * must list units (unit view) or activities (lesson view). Works on any entry.
   */
  getData_teacherPlayer: async function () {
    await logger.logInto(await stackTrace.get());
    var pe = selectorFile.css.ComproC1.practiceExtra;
    var onRoute = true == (await action.waitForUrl(/\/learning-path\/teacher\//, 60000));
    var playerShown = onRoute && true == (await action.waitForDisplayed(pe.activityTitleBtn, 60000));
    var out = { onRoute: onRoute, playerShown: playerShown, tocOpenedByUser: false, tocShown: false, unitShown: false, activityTitle: null };
    if (!playerShown) return out;
    out.activityTitle = await action.getAttribute(pe.activityTitleBtn, "title");
    // First entry: the TOC may still be mounting — give it the same 10 s as a learner's before opening it.
    if (true != (await action.waitForDisplayed(pe.tocSidebar, 10000))) {
      out.tocOpenedByUser = true;
      if (true == (await action.click(pe.tocOpenBtn))) await action.waitForDisplayed(pe.tocSidebar, 10000);
    }
    out.tocShown = true == (await action.isDisplayed(pe.tocSidebar));
    // "Content" = a unit row (unit view) or an activity row (lesson view).
    out.unitShown = out.tocShown && (true == (await action.waitForDisplayed(pe.tocUnitItem, 5000))
      || true == (await action.isDisplayed(pe.tocAnyActivity)));
    return out;
  },
