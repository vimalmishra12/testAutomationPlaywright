...
module.exports = {
  ...
  waitForCloneSuccess: async function () {
    await logger.logInto(await stackTrace.get());
    var res = await action.waitForExist(this.dialog, 15000, true);
    if (true === res) {
    ...
    // Dialog still open — wait briefly for inline error (server response may take a moment).
    var errorVisible = await action.waitForDisplayed(this.errorMsg, 5000);
  ...
  isInListing: async function (code, searchHint, checkAbsence) {
    ...
    // When verifying absence after deletion: poll every 5s for up to 30s to allow server propagation.
